...
@app.route('/table/<male_female>/<int:age>')
def table(male_female, age):
    if male_female == 'male':
        if age < 21:
            img_url = url_for('static', filename='img/small_alien.png')
            color = 'PaleTurquoise'
        else:
            img_url = url_for('static', filename='img/big_alien.png')
            color = 'Aqua'
    else:
        if age < 21:
            img_url = url_for('static', filename='img/small_alien.png')
    ...
    return render_template('table_cabin.html', img_url=img_url, color=color)


@app.route('/distribution')
def distribution():
    user_list = ['Кирилл', 'Венера', 'Алексей', 'Ваня']
